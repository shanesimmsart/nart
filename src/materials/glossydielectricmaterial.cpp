...
GlossyDielectricMaterial::GlossyDielectricMaterial(const glm::vec3& rho_s,
                                                   float eta, float alpha)
    : rho_s(rho_s), eta(eta), alpha(alpha) {}
...
BSDF GlossyDielectricMaterial::CreateBSDF(const glm::vec3& n, float alphaTweak,
  ...
  if (alpha_prime > 0.0001f) {
    BxDF* specularBRDF = new (memoryArena.Allocate(sizeof(TorranceSparrowBRDF)))
        TorranceSparrowBRDF(rho_s, eta, glm::max(0.0001f, alpha), alpha_prime);
