...
TriMeshPtr Scene::LoadMeshFromFile(const std::string& filePath,
  ...
  for (uint32_t i = 0; i < maxNormIndex + 1; ++i) {
    ...
    norms[i] = glm::normalize(glm::vec3(
        glm::inverse(((objectToWorld))) *
        glm::vec4(normCoords[j], normCoords[j + 1], normCoords[j + 2], 0.f)));
    j += 3;
  }
      ...
      for (auto& mat : elem["material"]) {
        try {
          ...
          if (type == "lambert") {
            ...
            meshMaterials.push_back(
                std::make_unique<GlassMaterial>(rho_s, tau, eta, alpha));
          }
            ...
            float alpha = roughness * roughness;
            meshMaterials.push_back(
                std::make_unique<GlossyDielectricMaterial>(rho_s, eta, alpha));
          }
            ...
            float alpha = roughness * roughness;
            meshMaterials.push_back(
                std::make_unique<PlasticMaterial>(rho_d, rho_s, eta, alpha));
          }
    ...
    for (auto& elem : json["lights"]) {
      ...
      try {
        ...
        if (type == "distant") {
          ...
          lights.emplace_back(
              std::make_unique<DistantLight>(Le, intensity, lightToWorld));
        }
        ...
        if (type == "disk") {
          ...
          glm::vec3 Le = glm::vec3(LeGet[0], LeGet[1], LeGet[2]);
          lights.emplace_back(
              std::make_unique<DiskLight>(radius, Le, intensity, lightToWorld));
        }
