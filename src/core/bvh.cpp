...
bool BVH::Chunk::Intersect(const Ray& ray, Intersection& isect) const {
  ...
  for (const TriangleIndex& triangleIndex : triangleIndices) {
    if (triangleIndex.mesh.triangles[triangleIndex.index].Intersect(ray,
                                                                    isect)) {
      hit = true;
...
bool BVH::Octree::Intersect(const Ray& ray, Intersection& isect) {
  ...
  while (!queue.empty()) {
    ...
    // If the nearest triangle is closer than the nearest bounding volume in the
    // queue, we don't need to continue
...
BVH::BVH(std::vector<TriMeshPtr>&& _meshes) : meshes(std::move(_meshes)) {
  ...
  glm::vec3 resolution = glm::floor(
      sceneSize *
      glm::pow(
          (glm::vec3(static_cast<float>(numTriangles)) / sceneVolume) * lambda,
          glm::vec3(1.f / 3.f)));
