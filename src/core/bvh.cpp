...
BVH::BoundingVolume::BoundingVolume() {
    // Bounding volumes are defined by slabs along the cardinal direction
    // vectors
...
BVH::Octree::OctreeNode::OctreeNode(const glm::vec3& nodeMin,
                                    const glm::vec3& nodeMax)
    : nodeMin(nodeMin), nodeMax(nodeMax) {}

BVH::Octree::Octree(const glm::vec3& sceneMin, const glm::vec3& sceneMax)
    : sceneMin(sceneMin), sceneMax(sceneMax) {
...
bool BVH::Octree::Intersect(const Ray& ray, Intersection& isect) {
    ...
    std::priority_queue<NodeDistancePair, std::vector<NodeDistancePair>,
                        std::greater<NodeDistancePair>>
        queue;
    ...
    while (!queue.empty()) {
        ...
        // If the nearest triangle is closer than the nearest bounding volume in
        // the queue, we don't need to continue
...
void BVH::Octree::OctreeNode::InsertChunk(ChunkPtr chunk, uint8_t depth) {
    if (isLeaf) {
        ...
        glm::vec3 chunkCentroid =
            chunk->bboxMin + ((chunk->bboxMax - chunk->bboxMin) * 0.5f);
        ...
        if (!children[nodeIndex]) {
            ...
            if (nodeIndex & 1)
                childNodeMax.x += childNodeSize.x;
            else
                childNodeMin.x -= childNodeSize.x;
            if (nodeIndex & 2)
                childNodeMax.y += childNodeSize.y;
            else
                childNodeMin.y -= childNodeSize.y;
            if (nodeIndex & 4)
                childNodeMax.z += childNodeSize.z;
            else
                childNodeMin.z -= childNodeSize.z;

            children[nodeIndex] =
                OctreeNodePtr(new OctreeNode(childNodeMin, childNodeMax));
        }
...
BVH::BVH(std::vector<TriMeshPtr>&& _meshes) : meshes(std::move(_meshes)) {
    ...
    // Cleary et al. 1983. Design and analysis of a parallel ray tracing
    // computer. In their formula, lambda controls the granularity of the grid;
    ...
    glm::vec3 resolution = glm::floor(
        sceneSize *
        glm::pow((glm::vec3(static_cast<float>(numTriangles)) / sceneVolume) *
                     lambda,
                 glm::vec3(1.f / 3.f)));

    resolution = glm::clamp(resolution, glm::vec3(1.f), glm::vec3(128.f));
    uint32_t numChunks =
        static_cast<uint32_t>(resolution.x * resolution.y * resolution.z);
    ...
    for (const TriMeshPtr& mesh : meshes) {
        for (uint32_t i = 0; i < mesh->GetNumTriangles(); ++i) {
            ...
            uint32_t chunkIndex = static_cast<uint32_t>(
                glm::floor(chunkCoords.x * resolution.y * resolution.z +
                           chunkCoords.y * resolution.z + chunkCoords.x));
