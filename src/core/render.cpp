...
#define DEBUG_BUCKET_X 21
#define DEBUG_BUCKET_Y 14

RenderSession::RenderSession(IntegratorPtr integrator, const Scene& scene,
                             RenderParams params)
    : integrator(std::move(integrator)), scene(scene), params(params) {
...
void RenderSession::AddSample(const float* filterTable,
    ...
    for (uint32_t y = y0; y < y1; ++y) {
        for (uint32_t x = x0; x < x1; ++x) {
            ...
            // Get index into precomputed filter table based on distance from
            // sample, as opposed to: float filterWeight =
            // Gaussian(params.filterWidth, dist);
            uint8_t filterIndex =
    ...
    for (uint32_t y = y0; y < y1; ++y) {
        for (uint32_t x = x0; x < x1; ++x) {
            ...
            for (uint32_t i = 0; i < params.spp; ++i) {
                ...
                glm::vec4 Li_alpha =
                    integrator->Li_alpha(rng, ray, scene, params, memoryArena);

                // Transform image sample to "total" image coords (image
                // including filter bounds)
                glm::vec2 sampleCoords = glm::vec2(
                    static_cast<float>(x + filterBounds) + imageSample.x,
                    static_cast<float>(y + filterBounds) + imageSample.y);
                // Add sample to pixels within filter width
                AddSample(filterTable, sampleCoords, Li_alpha, pixels);
    ...
    for (uint32_t y = 0; y < nBucketsY; ++y) {
        for (uint32_t x = 0; x < nBucketsX; ++x) {
            ...
            tg.run([this, &filterTable, &tiles, index, x, y, &nBucketsComplete,
                ...
                uint32_t y1 =
                    glm::min(params.bucketSize * (y + 1), totalHeight);
                std::vector<Pixel> tile =
                    RenderTile(filterTable, x0, x1, y0, y1);
                tiles[index] = tile;
                nBucketsComplete++;
            });
        }
            ...
            for (uint32_t y = 0; y < tileSize; ++y) {
                for (uint32_t x = 0; x < tileSize; ++x) {
                    ...
                    if (pX < (params.imageWidth + filterBounds) &&
                        ...
                        pixels[pIndex].contribution +=
                            v[(y * tileSize) + x].contribution;
    ...
    for (uint8_t i = 3; i < argc; ++i) {
        ...
        if ((arg == "--imageWidth" || arg == "-w") && argc > i) {
            ...
            try {
                ...
                std::cerr << "Invalid roughening factor: " << argv[i]
                          << std::endl;
                return false;
            }
        }
...
std::vector<std::unique_ptr<RenderSession>> LoadSessions(
    ...
    if (!json["renderSessions"].is_null()) {
        for (auto& elem : json["renderSessions"]) {
            ...
            if (!elem["integrator"].is_null() &&
                params.integratorFlag == PATH) {
                std::string integratorString =
                    elem["integrator"].get<std::string>();
                if (integratorString == "path") params.integratorFlag = PATH;
                if (integratorString == "volume")
                    params.integratorFlag = VOLUME;
            }
            ...
            if (!elem["rougheningFactor"].is_null() &&
                ...
                params.rougheningFactor = glm::min(
                    glm::max(elem["rougheningFactor"].get<float>(), 0.f), 1.f);
            ...
            switch (params.integratorFlag) {
                case 0:
                    integrator = std::make_unique<PathIntegrator>();
                    break;
                case 1:
                    integrator = std::make_unique<VolumeIntegrator>();
                    break;
            }

            sessions.push_back(std::make_unique<RenderSession>(
                std::move(integrator), scene, params));
        }
    }
