...
Ray PinholeCamera::CastRay(const glm::vec2& imageSample,
    ...
    float px = (((static_cast<float>(imageX) + imageSample.x) /
                 static_cast<float>(imageWidth)) *
                    2.f -
                1.f) *
               glm::tan(glm::radians(fov)) * aspectRatio;
    float py = (((static_cast<float>(imageY) + imageSample.y) /
                 static_cast<float>(imageHeight)) *
                    -2.f +
                1.f) *
               glm::tan(glm::radians(fov));
